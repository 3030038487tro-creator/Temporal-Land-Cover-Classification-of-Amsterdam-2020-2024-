...
<td><strong>Figure 6.</strong> K-means classifications (2020 → 2024), shown as an animated GIF to illustrate year-on-year change.</td>
</tr>
<tr>
<td><img src="Figures/Amsterdam_Colored.gif" width="300" alt="Animated RF maps"/></td>
<td><strong>Figure 7.</strong> Random-Forest classifications (2020 → 2024), shown as an animated GIF to illustrate year-on-year change.</td>
</tr>
<tr>
<td><img src="Figures/Land_Cover_Change.png" width="1500" alt="Change maps"/></td>
<td><strong>Figure 8.</strong> Urban gain (red) and vegetation loss (yellow), 2020–2024. RF detects more change due to its sensitivity to subtle spectral shifts; K-means is more conservative and stable in its outputs.</td>
</tr>
<tr>
<td><img src="Figures/Feature_importance_from_the_Random_Forest_model.png" width="300" alt="Feature importance chart"/></td>
<td><strong>Figure 9.</strong> Feature importance from the Random Forest model. NIR and SWIR2 dominate.</td>
</tr>
...
* **Urban spectral sub-types** The higher urban percentage in the K-means output (32.26%) reflects its ability to capture urban spectral sub-types that the Random Forest model (28.27%) overlooks. Specifically, K-means identifies high-reflectance industrial surfaces as a distinct structural cluster, whereas the supervised RF model—constrained by broader semantic labels—tends to underestimate these complex built-up zones by potentially misclassifying their edges or merging them with other high-reflectance features.
* **Vegetation assignment differs** The Random Forest model maps a higher percentage of vegetation. This is attributed to its sensitivity to NDVI-rich mixed pixels found in residential gardens and semi-urban fringes. K-means follows a stricter spectral clustering logic, which reduces the risk of over-classifying gray infrastructure as green space but may lead to the omission of shaded or sparse vegetation.
* **Hydrological stability** A notable source of divergence in the 'Water' class is the presence of wetlands and seasonally flooded areas around Amsterdam. These surfaces exhibit high absorption in the NIR/SWIR bands, mimicking the spectral signature of open water. Random Forest (19.15%) shows a tendency to group these saturated soils into the water category, likely inheriting the broad definitions from the ESA training labels. Conversely, K-means (12.69%) appears more conservative, potentially separating shallow, turbid, or vegetated water from the deep-water cluster, leading to a smaller but perhaps more spectrally pure water extent.
* **Change-detection divergence** The divergence in change detection highlights the inherent limitations of both approaches. The Random Forest model provides a more intuitive 'Growth' narrative but suffers from over-sensitivity to atmospheric noise, leading to a likely inflated urban gain. Conversely, the K-means model reveals the impact of spectral drift, where seasonal and sensor-induced variations cause cluster thresholds to shift, resulting in a false 'Urban decline.' This comparison suggests that for reliable long-term monitoring in Amsterdam, a hybrid approach—using K-means to filter stable training samples for a Random Forest model—would likely yield the most accurate results.
...
A short walk-through (6 min) covering data, code and results is available on YouTube:  
